...
import { fetchMyDonations } from "../../modules/donations/services/donationService";
import { getApiErrorMessage } from "../../utils/apiErrors";
import { getDonorDisplayName, getSessionUser } from "../../utils/authStorage";
import { dashboardAlertErrorClass } from "../../components/dashboard/dashboardFormStyles";
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) {
          setError(getApiErrorMessage(err, "Unable to load active donations."));
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
    ...
    >
      ...
      >
        <div className="relative pl-[0.5cm] pt-[0.5cm] pr-[0.5cm] pb-[0.5cm] sm:pr-[0.75cm] sm:pb-[0.75cm]">
          ...
          >
            <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[16px] bg-gradient-to-br from-[#16A34A] to-[#15803D] text-white shadow-[0_8px_24px_rgba(22,163,74,0.35)]">
          ...
          {error ? <p className={`mb-[0.5cm] ${dashboardAlertErrorClass}`}>{error}</p> : null}

          {loading ? (
            <p className="py-8 text-center text-[#64748B]">Loading active donations…</p>
          ) : (
            <ActiveDonationTracking donations={donations} showSectionHeader={false} />
          )}
        </div>
      </motion.section>
    </DashboardLayout>
