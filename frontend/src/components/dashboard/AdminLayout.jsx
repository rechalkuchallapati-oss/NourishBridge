...
import { getSessionUser, logoutDonor } from "../../utils/authStorage";
...
const CARD_TINTS = {
  ...
  indigo: {
    card: "border-[#E0E7FF] bg-gradient-to-br from-[#EEF2FF] to-white",
    icon: "border-[#C7D2FE] bg-[#E0E7FF] text-[#4338CA]",
  },
};

export function AdminStatCard({ label, value, caption, icon: Icon, accent = "indigo" }) {
  const tint = CARD_TINTS[accent] ?? CARD_TINTS.indigo;

  return (
    <article
      className={[
        "rounded-none border p-[0.5cm] shadow-[0_4px_20px_rgba(15,23,42,0.04)]",
        "transition-all duration-300 ease-out hover:-translate-y-[3px] hover:shadow-[0_12px_32px_rgba(15,23,42,0.1)]",
        tint.card,
      ].join(" ")}
    >
      {Icon ? (
      ...
      {caption ? (
        <p className="mt-[0.3cm] text-[10px] leading-5 text-[#64748B] sm:text-xs">{caption}</p>
      ) : null}
    </article>
  );
}
...
export default function AdminLayout({ children }) {
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#EEF2FF]/40 via-[#F8FAFC] to-white">
      <header className="border-b border-[#E5E7EB] bg-white/95 backdrop-blur-sm">
        <Container className="flex h-16 items-center justify-between gap-4 sm:h-[72px]">
          ...
          <div className="flex items-center gap-3">
            <span className="hidden items-center gap-2 rounded-none border border-[#E0E7FF] bg-[#EEF2FF] px-3 py-1.5 text-xs font-semibold text-[#4338CA] sm:inline-flex">
              <FaShieldAlt aria-hidden="true" />
              Admin
            </span>
            <p className="hidden max-w-[180px] truncate text-sm font-medium text-[#64748B] md:block">
              {getAdminDisplayName()}
            </p>
            <Link
              to="/login"
